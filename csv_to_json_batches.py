...
infile = Path(__file__).parent / "customerIds.csv"
...
for idx, batch in enumerate(batches, start=1):
    fname = out_dir / f"customerIds_batch_{idx:02d}.json"
    with fname.open("w", encoding="utf-8") as of:
        json.dump(batch, of, indent=2)
    created.append(str(fname.name))

manifest = out_dir / "customerIds_batches_manifest.json"
